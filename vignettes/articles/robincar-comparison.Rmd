---
title: "Generalized Linear Regression in RobinCar and RobinCar2"
output: rmarkdown::html_document
vignette: >
  %\VignetteIndexEntry{Generalized Linear Regression in RobinCar and RobinCar2}
...
RobinCar2 has a testing suite that ensures that the outputs are identical between RobinCar and RobinCar2. The comparisons use the `glm_data` dataset included in RobinCar2. The outputs for the point estimates and variance estimates are compared under a variety of settings, including:
...
Here we provide an example of using a *logistic regression working model* with *stratified permuted block randomization*, and the `glm_data` provided in the package. We output the treatment contrast.
...
Here we list major differences in the features that are available in RobinCar and RobinCar2. This will be continually updated as the packages are updated.
...
RobinCar supports many other methods like machine-learning methods. Those methods is not included in RobinCar2.
