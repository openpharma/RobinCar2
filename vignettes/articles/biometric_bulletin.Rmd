...
author: |
  ...
  | Daniel Sabanés Bové^5^, Ting Ye^6^, Yanyao Yi^7^
  |
...
- `vcovHC`: The Huber-White sandwich estimator, which is appropriate when treatment-by-covariate interactions are not included in the model for continuous outcomes
...
The output of `robin_surv()` again has a similar structure compared to `robin_lm()` and `robin_glm()`. The section **Contrast** provides unconditional (marginal) hazard ratios for all pairwise treatment comparisons. The section **Test** provides the logrank test results. The confidence interval of treatment effect contrasts can be obtained via the `confint()` function.
