...
This paper provides an introduction to **RobinCar2**, covering its core functionality for three common outcome types in clinical trials: continuous, binary, and time-to-event outcomes. It also provides best practices for using **RobinCar** and **RobinCar2** packages for covariate adjustment.
...
- **`vcovHC`**: The Huber-White sandwich estimator, which is appropriate only when treatment-by-covariate interactions are not included in the model for continuous outcomes.
...
**RobinCar2** provides a validated, user-friendly implementation of covariate adjustment methods for randomized clinical trials. Its simple interface, based on familiar R formula syntax, makes it accessible to clinical trial statisticians while ensuring robust inference aligned with FDA guidance. The package covers the most common outcome types encountered in clinical trials: continuous, binary, and time-to-event outcomes. Future development of **RobinCar2** may include additional variance estimation methods, the Mantel-Haenszel risk difference estimator, and bootstrap methods.
