...
derived_outcome_vals <- function(df, treatment, time, status, covariates, n = nrow(df)) {
    ...
    for (i in df$index) {
        ...
        # Careful here: we need to check the patient index, not the event time, which could tied.
        dN_ij <- delta_i * (df_events$index == i)
...
We can also see that the $\hat{O}_{ij}$ values here are *not* the same for the patients with tied event times.
...
where here the question comes up whether the regression coefficients are really fixed, or should be a function of $\vartheta$ too.
