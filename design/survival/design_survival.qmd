...
h_lr_score_no_strata_no_cov <- function(theta, df, treatment, time, status, n = nrow(df)) {
    ...
    for (i in seq_len(nrow(df_events))) {
        ...
        sigma_L2_factor_ti <- if (n_events_ti == 1) {
            1
        } else {
...
So also this matches now well (after we included the events factor above)
...
We can see that the point estimate matches very well, only the standard error is slightly different:
...
So this is the same as with `RobinCar`, and our results differ, so we need to fix that.
