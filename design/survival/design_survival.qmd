...
1. For the future, which kind of marginal means could we display? E.g. restricted mean survival times which are also covariate and stratification adjusted would be great. 
    ...
    - Note that this can come later of course. Was just an early idea 🙂 
...
1. In `RobinCar` the `adjust.CovHR()` method is not consistent with `adjust.LogRank()` regarding the handling of ties: In [line 85-87](https://github.com/mbannick/RobinCar/blob/main/R/adjust-covhr.R#L85) we don't have the correction factor that we have in the log-rank test. We should probably use the correction factor for consistency?
    - The only downside is that then we are no longer giving the same result as `surviva::coxph()`.
...
h_lr_score_no_strata_no_cov <- function(theta, df, treatment, time, status, n = nrow(df)) {
    ...
    df_events <- left_join(df_events, df_events_unique, by = "time")
    ...
    for (i in seq_len(nrow(df_events))) {
        ...
        sigma_L2 <- sigma_L2 + Y_bar_1_ti_coxph * Y_bar_0_ti * sigma_L2_factor_ti / Y_bar_all_ti^2
    }
...
Here we see a difference to our results, because we don't use the approximate method with `theta_hat` in the score function. The results are very similar, though:
...
So also this matches now well (after we included the events factor above), on all the statistics reported:
...
stopifnot(all.equal(
    sqrt(robincar2_result$test_sigma_L2 /
...
Since we don't use the approximate method with `theta_hat` in the score function, we get a slightly different value here. The results are very similar, though.
...
So here the results differ slightly:
...
Let's see whether it is because of the strata 3 with only a single male patient:
...
We still see the same relative difference, so it is not just because of this too small stratum.
...
So the `survival` result matches the `RobinCar` result, but not our result, so we need to fix that.
