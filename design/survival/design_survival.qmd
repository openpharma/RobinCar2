...
Similarly as above, we can write down the score function from the partial likelihood for the stratified Cox model with log hazard ratio $\theta$:
...
where $\widehat{\theta}_{\text{SL}}$ is the maximum partial likelihood estimate of the unadjusted stratified log hazard ratio, which we e.g. above implemented in `strat_log_rank`.
...
strat_covadj_log_rank_score <- function(theta, df, treatment, time, status, strata, model, theta_hat = NULL) {
    ...
    unadj_strat_res_score <- strat_log_rank_score(theta, df_included, treatment, time, status, strata)
    ...
    # Compute standard error for theta estimate.
    g_theta_SL <- unadj_strat_res$sigma_SL2
    var_theta_CSL <- (g_theta_SL - sigma_SL2_adj_term) / (g_theta_SL^2) / n
...
Let's do the root-finding to get the estimate:
