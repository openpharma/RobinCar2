...
h_lr_score_strat <- function(theta, df, treatment, time, status, strata) {
    ...
    # We use here the standard names now, instead of U_SL etc, to be able to use the generic
    # functions.
    U_L <- sum(sapply(strata_results, as.numeric))
    sigma_L2 <- sum(sapply(strata_results, attr, "sigma_L2"))
    se_theta_L <- sqrt(1 / (n * sigma_L2))
    structure(
        U_L,
        sigma_L2 = sigma_L2,
