...
```{r}
strat_dat_derived <- strat_derived_outcome_vals(dat, "sex", "time", "status", "strata", covariates = c("age", "ph.karno", "meal.cal"))
...
strat_covadj_log_rank <- function(df, treatment, time, status, strata, model) {
    ...
    # Calculate derived outcomes and regress them on covariates.
    df_split_with_covs_ovals <- strat_derived_outcome_vals(df, treatment, time, status, strata = strata, covariates = all.vars(model))
...
our_strat_covadj_res <- strat_covadj_log_rank(
    dat,
    treatment = "sex", time = "time", status = "status", strata = "strata",
    model = ~ age + ph.karno + meal.cal
)
