---
title: "Initial Code for Survival Analysis with RobinCar2"
author: Daniel Sabanes Bove
bibliography: references.bib
number-sections: true
---

The idea of this initial code prototyping was to code (almost) from scratch to have an independent implementation to compare with RobinCar, to really improve reliability. So I might still look at the overall flow of RobinCar and the helper functions, but I will not copy any code directly.
...
There are $2 \times 2 \times 2 = 8$ different analyses we can implement here, based on the combinations of:

- Covariate adjusted vs. unadjusted
- Stratified vs. unstratified
- Log-rank test vs. hazard ratio estimate

For each of them we have a corresponding section below.
...
However, any rows with missing values in any of the required variables are omitted.
...
derived_outcome_vals <- function(df, treatment, time, status, covariates, n = nrow(df)) {
    ...
    # Standardize data set format, subset to relevant variables and full observations.
    df <- data.frame(
...
We can see that the original patient number 5 has been omitted because of missing values in the covariates:
...
covadj_log_rank <- function(df, treatment, time, status, model) {
    ...
    # Obtain unadjusted result for the patients included.
...
So these results are very close to what we obtained earlier:
...
The standard error is numerically equal, but the estimate is still a little bit further off.
