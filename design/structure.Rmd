...
The class of the fit is `lm_std`, with similar printing like `lm`, including:
...
When using `summary(fit, vcov = "HC3")` to summarize the fit, it will print additional informations like

1. the function call
1. formula used
1. treatment variable
1. treatment effect and sd/df
1. significance

The inference is based on the robust covariance.

For weighting methods, potentially there is an additional formula to specify the weights.
This can be a wrapper around `ipw` and provide similar results.
