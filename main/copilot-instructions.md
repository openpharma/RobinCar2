...
`RobinCar2` is an R package for **robust covariate adjustment** in
randomized clinical trials. It provides methods for estimating and
inferring treatment effects under stratified randomization, aligned with
...
| Function | Purpose |
|----|----|
| [`robin_lm()`](https://openpharma.github.io/RobinCar2/reference/robin_lm.md) | Linear model covariate adjustment (ANCOVA) |
| [`robin_glm()`](https://openpharma.github.io/RobinCar2/reference/robin_glm.md) | GLM-based covariate adjustment (logistic, Poisson, negative binomial) |
| [`robin_surv()`](https://openpharma.github.io/RobinCar2/reference/robin_surv.md) | Survival analysis with covariate adjustment |

### Key Concepts

- **Treatment formulas**: Use `treatment ~ strata` syntax (e.g.,
  `treatment ~ s1` or `treatment ~ pb(s1)` for permuted-block)
- **Randomization schemas**: `sr` (simple), `pb` (permuted-block), `ps`
  (Pocock-Simon)
- **Variance estimators**: `vcovG` (default, robust) and `vcovHC`
  (Huber-White, limited use cases)
- **Contrasts**: `"difference"`, `"risk_ratio"`, `"odds_ratio"`,
  `"log_risk_ratio"`, `"log_odds_ratio"`
...
- **Internal helpers**: `h_*()` prefix (e.g.,
  [`h_get_vars()`](https://openpharma.github.io/RobinCar2/reference/h_get_vars.md),
  [`h_interaction()`](https://openpharma.github.io/RobinCar2/reference/h_interaction.md))
...
`assert_subset``(`[`all.vars`](https://rdrr.io/r/base/allnames.html)`(``formula``)``, `[`names`](https://rdrr.io/r/base/names.html)`(``data``)``)`\
`assert_function``(``contrast``, args ``=`` `[`c`](https://rdrr.io/r/base/c.html)`(``"x"``, ``"y"``)``)`

### Testing (testthat v3)

- Test file naming: `test-<function_name>.R`
- Use `expect_silent()`, `expect_error()`, `expect_warning()` for
  behavior tests
...
\
`  ``expect_silent``(`\
`    `[`robin_glm`](https://openpharma.github.io/RobinCar2/reference/robin_glm.md)`(``y`` ``~`` ``treatment`` ``*`` ``s1``, data ``=`` ``glm_data``, treatment ``=`` ``treatment`` ``~`` ``s1``, contrast ``=`` ``"difference"``)`\
`  ``)`\
`}``)`
...
1.  **Design first**: Review relevant documents in `design/` for
    architectural context
2.  **Implement**: Create/modify files in `R/`, following existing
    patterns
3.  **Document**: Add roxygen2 comments; run
...
- `vcovHC`: Huber-White (only for linear models without interactions,
  difference contrast)
