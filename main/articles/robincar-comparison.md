...
`if`` ``(``!`[`requireNamespace`](https://rdrr.io/r/base/ns-load.html)`(``"RobinCar"``, quietly ``=`` ``TRUE``)``)`` ``{`` `` `[`install.packages`](https://rdrr.io/r/utils/install.packages.html)`(``"RobinCar"``)`` ``}`` ``#> Installing package into '/usr/local/lib/R/site-library'`` ``#> (as 'lib' is unspecified)`` ``#> also installing the dependencies 'gtools', 'gplots', 'listenv', 'gargle', 'ids', 'rematch2', 'ROCR', 'future', 'truncnorm', 'quadform', 'conflicted', 'dtplyr', 'googledrive', 'googlesheets4', 'reprex', 'gam', 'nnls', 'cvAUC', 'future.apply', 'progressr', 'Rsolnp', 'emulator', 'tidyverse', 'fastDummies', 'SuperLearner', 'AIPW'`` `[`library`](https://rdrr.io/r/base/library.html)`(`[`RobinCar2`](https://github.com/openpharma/RobinCar2/)`)`` ``#> `` ``#> Attaching package: 'RobinCar2'`` ``#> The following object is masked from 'package:base':`` ``#> `` ``#> table`` `[`library`](https://rdrr.io/r/base/library.html)`(``RobinCar``)`
