...
`#> also installing the dependencies 'gtools', 'gplots', 'listenv', 'gargle', 'ids', 'rematch2', 'ROCR', 'future', 'truncnorm', 'quadform', 'conflicted', 'dtplyr', 'googledrive', 'googlesheets4', 'reprex', 'gam', 'nnls', 'cvAUC', 'future.apply', 'progressr', 'Rsolnp', 'emulator', 'tidyverse', 'fastDummies', 'SuperLearner', 'AIPW'`\
...
`  g_family ``=`` `[`binomial`](https://rdrr.io/r/stats/family.html)`(``link ``=`` ``"logit"``)``,`\
`  contrast_h ``=`` ``"diff"`\
`)`\
`#> $main`\
...
`#>   ``<chr>``    ``<dbl>``  ``<dbl>``            ``<dbl>`\
`#> ``1`` pbo      0.356 0.033``6``         3.03``e``-26`\
`#> ``2`` trt1     0.581 0.034``4``         7.25``e``-64`\
`#> ``3`` trt2     0.621 0.034``0``         1.54``e``-74`\
`#> `\
...
`#>   ``<chr>``               ``<dbl>``  ``<dbl>``            ``<dbl>`\
`#> ``1`` treat trt1 - pbo    0.225 0.047``7``     0.000``002``51  `\
`#> ``2`` treat trt2 - pbo    0.265 0.047``5``     0.000``000``023``9`\
`#> `\
